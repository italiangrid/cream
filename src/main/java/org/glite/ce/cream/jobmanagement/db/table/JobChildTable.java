...
public class JobChildTable implements JobChildTableInterface {
    ...
    private final static Logger logger = Logger.getLogger(JobChildTable.class);

    public JobChildTable() throws SQLException {
        logger.debug("Call JobChildTable constructor");
    }

    public int executeInsert(String jobId, List<String> childJobIdList, Connection connection) throws SQLException {
        logger.debug("Begin executeInsert");
        // logger.debug("insertQuery (JobChildTable)= " + insertQuery);
        ...
        try {
            ...
            for (String childJobId : childJobIdList) {
                insertPreparedStatement = fillInsertPreparedStatement(jobId, childJobId, insertPreparedStatement);
    ...
    public List<String> executeSelect(String jobId, Connection connection) throws SQLException {
        ...
        List<String> childJobList = new ArrayList<String>(0);
        String[] childJob = null;
        // logger.debug("selectQuery (JobChildTable)= " + selectQuery);
        ...
        try {
            ...
            while (resultSet.next()) {
                childJobList.add(resultSet.getString(JobChildTable.ID_FIELD));
            }
        } catch (SQLException sqle) {
            throw sqle;
        } finally {
    ...
    public int executeDelete(String jobId, Connection connection) throws SQLException {
        ...
        // logger.debug("deleteQuery (JobChildTable)= " + deleteQuery);
    ...
    private PreparedStatement fillInsertPreparedStatement(String jobId, String childJob, PreparedStatement insertPreparedStatement) throws SQLException {
        insertPreparedStatement.setString(1, childJob);
    ...
    private static String getInsertQuery() {
        ...
        insertQuery.append(JobChildTable.NAME_TABLE);
        insertQuery.append(" ( ");
