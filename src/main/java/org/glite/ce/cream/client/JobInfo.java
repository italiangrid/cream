...
public class JobInfo extends JobCommand {

    public JobInfo(String[] args, List<String> options) throws RuntimeException {
        super(args, options);
    }

    public void execute() {
        JobInfoResult[] result = null;
        try {
            result = getCREAMStub().jobInfo(getJobIdList(), getStatus(), getFromDate(), getToDate(), getDelegationId(), getLeaseId());
        ...
        if (result != null) {
            for (int i = 0; i < result.length; i++) {
                ...
                if (result[i].getJobInfo() != null) {
                    printJobInfo(result[i].getJobInfo());
                } else {
                    ...
                    if (result[i].getLeaseIdMismatchFault() != null) {
