...
public class SetLease extends JobCommand {

    public SetLease(String[] args, List<String> options) throws RuntimeException {
        super(args, options);
    }

    public void execute() {
        if (getLeaseId() == null || getLeaseExpTime() == null) {
            printUsage();
        }

        try {
            Lease newLease = getCREAMStub().setLease(getLeaseId(), getLeaseExpTime());
            System.out.println("lease time: " + newLease.getLeaseTime().getTime());
    ...
    public static void main(String[] args) {
        ...
        options.add(LEASE_EXP_TIME);

        new SetLease(args, options);
    }
}
