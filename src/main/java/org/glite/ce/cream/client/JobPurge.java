...
public class JobPurge extends JobCommand {
    public JobPurge(String[] args, List<String> options) throws RuntimeException {
        super(args, options);
    }

    public void execute() {
        try {
            printResult(getCREAMStub().jobPurge(getJobIdList(), getStatus(), getFromDate(), getToDate(), getDelegationId(), getLeaseId()), "job being purged");
