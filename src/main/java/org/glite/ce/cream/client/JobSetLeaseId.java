...
public class JobSetLeaseId extends JobCommand {

    public JobSetLeaseId(String[] args, List<String> options) throws RuntimeException {
        super(args, options);
    }

    public void execute() {
        try {
            printResult(getCREAMStub().jobSetLeaseId(getNewLeaseId(), getJobFilter()), "leaseId being set");
    ...
    public static void main(String[] args) {
        ...
        options.add(NEW_LEASE_ID);
        options.add(FROM_DATE);
        options.add(TO_DATE);
        
        new JobSetLeaseId(args, options);
    }
}
